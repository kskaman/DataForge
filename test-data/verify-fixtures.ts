...
async function inspectScenario(scenario: ExpectedScenario) {
    ...
    return {
        datasetCount,
        rowCount,
        schemaGroupCount: schemas.size,
        faultCodes: [...faultCodes].sort(),
    }
}

async function main() {
    const manifest = JSON.parse(
        await readFile(path.join(root, 'expected-analysis.json'), 'utf8'),
    ) as Manifest

    for (const [name, scenario] of Object.entries(manifest.scenarios)) {
        const actual = await inspectScenario(scenario)

        if (scenario.expectedDatasets !== undefined)
            assert.equal(actual.datasetCount, scenario.expectedDatasets, `${name}: dataset count`)
        if (scenario.expectedSchemaGroups !== undefined)
            assert.equal(
                actual.schemaGroupCount,
                scenario.expectedSchemaGroups,
                `${name}: schema group count`,
            )
        if (scenario.expectedRows !== undefined)
            assert.equal(actual.rowCount, scenario.expectedRows, `${name}: row count`)
        if (scenario.expectedFaultCodes)
            assert.deepEqual(
                actual.faultCodes,
                [...scenario.expectedFaultCodes].sort(),
                `${name}: fault codes`,
            )
