...
orders.addRow(['ORD-3001', 'Precision sensor', 12, 149.95, 'North', new Date('2026-06-01T09:30:00.000Z'), true])
orders.addRow(['ORD-3002', 'Valve, stainless', 4, 87.5, 'South', new Date('2026-06-02T14:15:00.000Z'), false])
orders.addRow(['ORD-3003', 'Calibration kit', 1, 1200, 'West', new Date('2026-06-03T08:00:00.000Z'), true])
styleHeader(orders.getRow(1))
orders.columns.forEach((column) => { column.width = 20 })

const inventory = workbook.addWorksheet('Inventory Snapshot')
inventory.addRow(['SKU', 'Warehouse', 'On Hand', 'Reorder Point', 'Unit Cost', 'Last Counted', 'Notes'])
inventory.addRow(['SNS-001', 'Montréal', 84, 20, 72.4, new Date('2026-08-17T00:00:00.000Z'), 'Temperature controlled'])
inventory.addRow(['VLV-204', 'São Paulo', 7, 15, 31.25, new Date('2026-08-18T00:00:00.000Z'), 'Reorder immediately'])
inventory.addRow(['KIT-900', '東京', 0, 5, 640, new Date('2026-08-19T00:00:00.000Z'), 'Awaiting customs\nPriority shipment'])
styleHeader(inventory.getRow(1))
inventory.columns.forEach((column) => { column.width = 22 })
...
staff.addRow(['E-102', 'Léa Dubois', 'Data Quality', new Date('2023-09-01T00:00:00.000Z'), 0.8, true])
staff.addRow(['E-103', 'يوسف حمد', 'Logistics', new Date('2020-01-20T00:00:00.000Z'), 0.5, false])
styleHeader(staff.getRow(1))
staff.columns.forEach((column) => { column.width = 20 })

const formulas = workbook.addWorksheet('Formula Results')
formulas.addRow(['Metric', 'Expression', 'Value', 'Notes'])
formulas.addRow(['Gross value', 'Units * Unit Price', { formula: "'Orders 2026'!C2*'Orders 2026'!D2", result: 1799.4 }, 'Formula result is exported'])
formulas.addRow(['Open orders', 'COUNTIF(Fulfilled, false)', { formula: 'COUNTIF(\'Orders 2026\'!G2:G4,FALSE)', result: 1 }, 'ExcelJS reads the cached result'])
formulas.addRow(['Data label', 'Rich text', { richText: [{ text: 'High ', font: { bold: true } }, { text: 'priority', font: { color: { argb: 'FFB42318' } } }] }, 'Rich-text runs become one string'])
formulas.addRow(['Reference', 'Hyperlink', { text: 'DataForge docs', hyperlink: 'https://example.invalid/dataforge' }, 'Hyperlinks export their display text'])
styleHeader(formulas.getRow(1))
formulas.columns.forEach((column) => { column.width = 28 })
...
await writeFile(path.join(errorDirectory, 'corrupt-workbook.xlsx'), 'This is deliberately not an XLSX ZIP container.\n', 'utf8')
...
if (process.argv.includes('--large')) {
  ...
  async function writeSizedCsv(name, sizeInBytes, marker) {
    ...
    await writeFile(path.join(largeDirectory, name), header + prefix + 'x'.repeat(payloadLength) + suffix, 'utf8')
  }

  await writeSizedCsv('over-50mb.csv', 50 * megabyte + 1, 'TOO-LARGE')
  await Promise.all(Array.from({ length: 5 }, (_unused, index) =>
    writeSizedCsv(`aggregate-${index + 1}.csv`, 41 * megabyte, `AGG-${index + 1}`),
  ))
}

console.log(`Generated DataForge fixtures${process.argv.includes('--large') ? ' including large limit files' : ''}.`)