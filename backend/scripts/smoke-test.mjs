...
if (!body.jobs.some((item) => item.id === job.id))
    throw new Error('Guest history did not retain the completed job.')

response = await otherGuest.fetch('/api/jobs')
body = await response.json()
if (body.jobs.length !== 0)
    throw new Error(`Second guest saw another guest's history: ${JSON.stringify(body)}`)
...
if (response.status !== 404)
    throw new Error(`Cross-guest signed job download returned ${response.status}.`)
...
workbook.addWorksheet('North').addRows([
    ['name', 'amount'],
    ['Alpha', 120],
])
workbook.addWorksheet('South').addRows([
    ['name', 'amount'],
    ['Beta', 85],
])
...
if (archive[0] !== 0x50 || archive[1] !== 0x4b)
    throw new Error('Workbook result is not a ZIP archive.')
...
if (response.status !== 404)
    throw new Error(`Cross-guest batch lookup returned ${response.status}.`)
...
if (batch.status !== 'completed')
    throw new Error(`Merged batch ended as ${batch.status}: ${JSON.stringify(batch.faults)}`)
...
if (response.status !== 404)
    throw new Error(`Cross-guest signed batch download returned ${response.status}.`)
...
batch = await waitForBatch(guest, batch, ['completed', 'failed'])
if (batch.status !== 'completed')
    throw new Error(`SQLite batch ended as ${batch.status}: ${JSON.stringify(batch.faults)}`)
response = await guest.fetch(`/api/batches/${batch.id}/download`)
...
if (sqliteHeader !== 'SQLite format 3\u0000')
    throw new Error('Batch result is not a SQLite database.')
...
if (
    batch.status !== 'failed' ||
    batch.faults.length !== 3 ||
    !batch.faults.some((fault) => fault.code === 'unsupported_file')
) {
...
if (response.status !== 413 || body.error !== 'Choose no more than 20 files.') {
    throw new Error(
        `Expected batch file-count rejection: ${response.status} ${JSON.stringify(body)}`,
    )
}
