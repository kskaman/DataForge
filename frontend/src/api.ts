...
export type BatchJob = {
  ...
  faults: BatchFault[]
  configuration: { strategy: BatchStrategy; format?: OutputFormat; sqliteLayout?: 'per_source' | 'grouped' } | null
  resultFileName?: string
}
...
export async function configureBatch(
  id: string,
  configuration: { strategy: BatchStrategy; format?: OutputFormat; sqliteLayout?: 'per_source' | 'grouped' },
) {
  return (await request<{ batch: BatchJob }>(`/api/batches/${id}/configure`, {
    method: 'POST',
...
export async function retryBatch(id: string) {
  return (await request<{ batch: BatchJob }>(`/api/batches/${id}/retry`, { method: 'POST' })).batch
}
