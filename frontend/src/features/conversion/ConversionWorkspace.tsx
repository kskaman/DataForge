...
import { configureBatch, createBatch, type BatchJob, type BatchStrategy, type OutputFormat } from '../../api'
...
function validationError(files: File[]) {
  const invalid = files.find((file) => !['xlsx', 'csv'].includes(file.name.split('.').pop()?.toLowerCase() ?? ''))
  ...
  if (files.reduce((sum, file) => sum + file.size, 0) > MAX_BATCH_SIZE) return 'Batch size must be 200 MB or less.'
  return ''
}
...
export function ConversionWorkspace({ batches, onBatchChange, onError }: ConversionWorkspaceProps) {
  ...
  const analyzeFiles = async () => {
    if (selectedFiles.length === 0) return onError('Select one or more spreadsheets before analysis.')
  ...
  const createOutput = async () => {
    ...
    setIsSubmitting(true)
    try {
      const configuration = strategy === 'sqlite' ? { strategy, sqliteLayout } : { strategy, format }
  ...
  return (
    <section className="conversion-layout">
      ...
      <OutputSettings
        ...
        onSubmit={() => void (draftBatch?.status === 'awaiting_configuration' ? createOutput() : analyzeFiles())}
      />
    </section>
  )
}