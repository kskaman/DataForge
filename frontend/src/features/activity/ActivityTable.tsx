  ...
  return (
    <section className={historyView ? 'jobs-section history-view' : 'jobs-section'}>
      <div className="section-heading">
        <div><h2>{historyView ? 'All conversions' : 'Recent activity'}</h2><p>Updates automatically while files are processed</p></div>
        {!historyView && <button type="button" onClick={onViewAll}>View all</button>}
      </div>
          ...
          <tbody>
            {batches.map((batch) => (
              <tr key={`batch-${batch.id}`}>
                ...
                <td className="job-actions">
                  {batch.status === 'completed' && <button type="button" onClick={() => onDownloadBatch(batch.id)}>Download</button>}
                  {batch.status === 'failed' && <button type="button" onClick={() => onRetryBatch(batch.id)}>Retry</button>}
            ...
            {jobs.map((job) => (
              <tr key={job.id}>
                ...
                <td className="job-actions">
                  {job.status === 'completed' && <button type="button" onClick={() => onDownloadJob(job.id)}>Download</button>}
                  {job.status === 'failed' && <button type="button" onClick={() => onRetryJob(job.id)}>Retry</button>}
