    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <ActivityTable
          ...
          onDownloadJob={(id) => void runAction(() => activity.downloadJob(id), 'Download failed.')}
          onRetryJob={(id) => void runAction(() => activity.retryJob(id), 'Retry failed.')}
          onDownloadBatch={(id) => void runAction(() => activity.downloadBatch(id), 'Download failed.')}
          onRetryBatch={(id) => void runAction(() => activity.retryBatchJob(id), 'Batch retry failed.')}
        />
      </main>

      <footer><span>DataForge</span><span>Anonymous history is private to this browser for 24 hours.</span></footer>
    </div>
  )
}
